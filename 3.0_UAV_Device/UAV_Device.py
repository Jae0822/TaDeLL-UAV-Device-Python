...
class Device(object):
    def __init__(self, mu, sig, nTimeUnits=900, nTasks=3, edge=1000):
        ...
        self.TaskList_Natural = copy.deepcopy(self.TaskList)  # For the comparison without ward start
    ...
    def UAV_Fly(self, Devices, TaDeLL_Model, PolType, **param):
        ...
        while t < nTimeUnits:
            ...
            if device.flag and (not np.any(device.TimeList[device.KeyTime[-1]: t])):
                # it's first time and there's no more new task arrival
                device.flag = False
                ...
                if device.ta_dex > nTasks - 1:
                    print("index out!")  # FXIME: to debug an error. But little chance to recreate the bug
